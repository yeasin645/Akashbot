...
// --- ১. কনফিগারেশন (Render Environment Variables থেকে আসবে) ---
const config = {
    token: process.env.BOT_TOKEN,
    mongoUri: process.env.MONGODB_URI,
    adminId: parseInt(process.env.ADMIN_ID),
    adminUser: process.env.ADMIN_USERNAME, // আপনার টেলিগ্রাম ইউজারনেম (উদা: MyUser)
    appUrl: process.env.APP_URL, // আপনার অ্যাপের লিঙ্ক (উদা: https://bot.onrender.com)
    dbVersion: "1.0" 
};
...
function getMenu(chatId) {
    ...
    if (chatId === config.adminId) {
        btns.push(
            [{ text: "📊 পরিসংখ্যান (Admin)", callback_data: "view_stats" }, { text: "✨ প্ল্যান অ্যাড (Admin)", callback_data: "adm_add_plan" }],
...
bot.onText(/\/start/, async (msg) => {
    ...
    bot.sendMessage(msg.chat.id, "🎬 **Professional Movie Bot Panel**", { 
        parse_mode: 'Markdown', 
...
bot.on('callback_query', async (q) => {
    ...
    if (q.data === "add_p" && chatId === config.adminId) {
        ...
        let txt = "💎 **প্রিমিয়াম প্ল্যান তালিকা:**\n\n";
        plans.forEach(p => txt += `✅ ${p.title} - ${p.price} (${p.days} দিন)\n`);
        bot.sendMessage(chatId, txt || "কোনো প্ল্যান নেই।", { parse_mode: 'Markdown', reply_markup: ownerBtn });
    }
    else if (q.data === "start_post") { 
        userState[chatId] = { step: 'title', links: [] }; 
        bot.sendMessage(chatId, "🎬 মুভির নাম কি?"); 
    }
...
// --- ৫. ইনপুট হ্যান্ডলিং ও স্মার্ট মেম্বার অ্যাড ---
...
// --- ৬. অলওয়েজ অনলাইন (Anti-Sleep) ও সার্ভার ---
...
app.listen(PORT, () => {
    ...
    // বোটকে ঘুমিয়ে পড়া থেকে বাঁচাতে ৫ মিনিট পর পর সেলফ-পিঙ্গ (Render Anti-Sleep)
    if (config.appUrl) {
        cron.schedule('*/5 * * * *', () => {
